...
export const ModalDeleteProject = () => {
  ...
  const { project, updateDeleteUsersProject } = useProjects()


  const { loading, callEndpoint } = useFetchAndLoad();

  const deleteTaskProject = async (): Promise<void> => {
    try {
      await callEndpoint(deleteProjectService(project._id));
      updateDeleteUsersProject(project);
      handleModalDeleteProjectVisibility({});
    } catch (error: any) {
  ...
  return (
    <Transition.Root show={modalDeleteProjectVisibility} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-10 inset-0 overflow-y-auto"
        onClose={() => handleModalDeleteProjectVisibility({})}
      >
            ...
            <div className="inline-block align-bottom bg-white px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              <div className="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button
                  ...
                  onClick={() => handleModalDeleteProjectVisibility({})}
                >
              ...
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  >
                    <p className="text-4xl">Delete project</p>
                  </Dialog.Title>
                  <div>
                    <p>A deleted project cannot be recovered</p>
                  </div>
                </div>
              </div>
